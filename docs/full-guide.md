...
国内期货分析走独立的 `futures` 资产类型，用于商品期货和金融期货的趋势、支撑压力、量价结构、保证金/杠杆、换月和供需风险分析。它不会套用股票基本面、筹码分布、股东、估值等股票专属逻辑。

能力范围：

- 支持品种主力连续合约：输入 `RB`、`I`、`AU` 这类品种代码时会自动转换为 `RB0`、`I0`、`AU0` 等新浪主力连续合约。
- 支持具体月份合约：输入 `JM2609`、`焦煤2609` 这类具体合约时会保留合约月份并拉取具体合约日线。
- 支持中文品种与别名：例如 `螺纹钢`、`铁矿石`、`沪金`、`焦煤`，并覆盖国内主要交易所返回的期货候选。
- 支持多空双向建议：兼容字段仍为 `decision_type=buy/hold/sell`，但期货语境下分别解释为做多/观望/做空。

常用输入格式：

| 输入 | 说明 | 示例 |
|------|------|------|
| 品种代码 | 转为主力连续合约 | `RB`、`I`、`AU` |
| 具体合约代码 | 保留交割月份 | `JM2609`、`IF2606` |
| 中文品种 + 月份 | 通过别名识别具体合约 | `焦煤2609`、`螺纹钢2605` |
...
1. 启动 Web 服务后打开首页。
2. 在分析入口切换到“期货”。
3. 搜索 `焦煤2609`、`JM2609`、`IF2606` 或品种名称。
4. 首次加载期货候选时，前端会请求 `GET /api/v1/stocks/futures-index`；接口会汇总 AkShare 当前可交易期货候选，覆盖上期所、大商所、郑商所、广期所和中金所返回的品种、主力连续和具体合约。接口失败时前端回退到内置基础候选集。

API 调用时可在分析请求里传入 `asset_type=futures`：
...
期货报告中的交易建议采用期货语义：`buy` 表示做多或偏多，`hold` 表示观望或维持计划，`sell` 表示做空或偏空。Web 策略点位显示为“理想入场/二次入场”，避免把做空建议误写成股票卖出或减仓。
