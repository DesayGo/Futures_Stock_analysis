...
1. **Docker 镜像发布版本**：看你部署时使用的镜像 tag，例如 `ghcr.io/<GHCR_NAMESPACE>/daily_stock_analysis:v3.12.0`。仓库的 Docker 发布由 `.github/workflows/docker-publish.yml` 按 `v*.*.*` Git tag 触发，所以 Docker 版本应以镜像 tag / GitHub Releases 为准。
...
image: ghcr.io/<GHCR_NAMESPACE>/daily_stock_analysis:v3.12.0
```

```bash
# 方式 2：回看你的拉取命令
docker pull ghcr.io/<GHCR_NAMESPACE>/daily_stock_analysis:v3.12.0
```
...
遇到其他问题？欢迎 [提交 Issue](https://github.com/DesayGo/daily_stock_analysis/issues)。
